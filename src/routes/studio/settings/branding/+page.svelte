<script lang="ts">
	...
	type Config = { siteName?: string; tagline?: string; logoUrl?: string; faviconUrl?: string };
	let config = $state<Config>({ siteName: '', tagline: '', logoUrl: '', faviconUrl: '' });
	...
	async function save(e: Event) {
		...
		try {
			...
			if (!isHtml && data?.success) {
				success = 'Branding saved to config/branding.json. Use these values in your layout or SEO.';
			} else {
...
<svelte:head><title>Branding | Studio Settings</title></svelte:head>

<div class="settings-page">
	<div class="page-header">
		<h1>Branding</h1>
		<p class="page-description">
			Site name, tagline, logo and favicon URLs. Saved to <code>config/branding.json</code>. Use these in your layout or combine with SEO defaults.
		</p>
	</div>
	{#if loading}
	...
	{:else}
		<form onsubmit={save} class="settings-form">
			<div class="form-group">
				...
				<input id="siteName" type="text" bind:value={config.siteName} placeholder="e.g. QuantumCore Solutions" />
			</div>
			<div class="form-group">
				<label for="tagline">Tagline</label>
				<input id="tagline" type="text" bind:value={config.tagline} placeholder="e.g. Engineering clarity into complexity" />
			</div>
			<div class="form-group">
				<label for="logoUrl">Logo URL</label>
				<input id="logoUrl" type="url" bind:value={config.logoUrl} placeholder="https://... or /path/to/logo.svg" />
			</div>
			<div class="form-group">
				<label for="faviconUrl">Favicon URL</label>
				<input id="faviconUrl" type="url" bind:value={config.faviconUrl} placeholder="https://... or /favicon.svg" />
			</div>
