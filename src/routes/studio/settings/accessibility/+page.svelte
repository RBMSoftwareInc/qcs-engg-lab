<script lang="ts">
	...
	type Config = { skipLinkLabel?: string; a11yStatementUrl?: string };
	let config = $state<Config>({ skipLinkLabel: 'Skip to main content', a11yStatementUrl: '' });
	...
	async function save(e: Event) {
		...
		try {
			...
			if (!isHtml && data?.success) {
				success = 'Accessibility config saved to config/accessibility.json.';
			} else {
...
<svelte:head><title>Accessibility | Studio Settings</title></svelte:head>

<div class="settings-page">
	<div class="page-header">
		<h1>Accessibility</h1>
		<p class="page-description">
			Configure skip link text and optional link to your accessibility statement. Saved to <code>config/accessibility.json</code>. Helps with inclusivity and compliance.
		</p>
	</div>
	{#if loading}
	...
	{:else}
		<form onsubmit={save} class="settings-form">
			<div class="form-group">
				<label for="skipLinkLabel">Skip link label</label>
				<input id="skipLinkLabel" type="text" bind:value={config.skipLinkLabel} placeholder="Skip to main content" />
				<small>Text for the “skip to main content” link (keyboard and screen reader users).</small>
			</div>
			<div class="form-group">
				<label for="a11yStatementUrl">Accessibility statement URL</label>
				<input id="a11yStatementUrl" type="url" bind:value={config.a11yStatementUrl} placeholder="https://yoursite.com/accessibility" />
				<small>Optional. Link to your accessibility or WCAG statement (footer or dedicated page).</small>
			</div>
