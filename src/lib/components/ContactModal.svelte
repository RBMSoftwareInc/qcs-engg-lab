<script lang="ts">
	/** Optional copy from Studio Data Forms config (label, submitText, successMessage, field labels/placeholders) */
	...
	let { open = $bindable(false), copy } = $props<{ open?: boolean; copy?: ContactModalCopy }>();
	...
	const title = $derived(copy?.label ?? 'Contact us');
	const submitLabel = $derived(copy?.submitText ?? 'Send');
...
{#if open}
	<div class="modal-backdrop" onclick={handleBackdropClick} role="dialog" aria-modal="true" aria-labelledby="contact-modal-title">
		<div class="modal-content">
			...
			{:else}
				<p class="modal-description">Send us a message and we'll respond as soon as we can.</p>
				<form onsubmit={handleSubmit} class="modal-form">
					<div class="form-group">
						<label for="contact-name">{field('name', 'label') ?? 'Name'}</label>
						<input id="contact-name" type="text" bind:value={name} required autocomplete="name" placeholder={field('name', 'placeholder') ?? ''} />
					</div>
					<div class="form-group">
						<label for="contact-email">{field('email', 'label') ?? 'Email'}</label>
						<input id="contact-email" type="email" bind:value={email} required autocomplete="email" placeholder={field('email', 'placeholder') ?? ''} />
					</div>
					<div class="form-group">
						<label for="contact-message">{field('message', 'label') ?? 'Message'}</label>
						<textarea id="contact-message" bind:value={message} rows="4" required placeholder={field('message', 'placeholder') ?? 'Your message...'}></textarea>
					</div>
					{#if error}<div class="form-error" role="alert">{error}</div>{/if}
					<button type="submit" class="submit-btn" disabled={isSubmitting}>{isSubmitting ? 'Sending…' : submitLabel}</button>
				</form>
			{/if}
		</div>
	</div>
...
<style>
	...
	.form-group input, .form-group textarea { padding: 0.75rem; border: 1px solid var(--border-subtle); border-radius: 8px; background: var(--bg-primary); color: var(--text-primary); font: inherit; }
	.form-group input:focus, .form-group textarea:focus { outline: none; border-color: var(--highlight); }
